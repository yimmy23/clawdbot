...
describe("looksLikeMSTeamsTargetId", () => {
  // #58001: preserve each documented conversation format; #115609: provider prefixes.
  it.each([
    ...
    "A:1xyz-abc_def",
    "8:orgid:user-object-id",
    "29:1a2b3c4d5e6f",
    "user:40a1a0ed-4ff2-4164-a219-55518990c197",
    "  TEAMS:user:40a1a0ed-4ff2-4164-a219-55518990c197  ",
    "msteams:conversation:19:abc@thread.tacv2",
  ])("accepts explicit target %s", (raw) => {
    expect(looksLikeMSTeamsTargetId(raw)).toBe(true);
  });

  it.each(["   ", "user:John Smith", "msteams:user:John Smith", "Product Team/Roadmap"])(
    "rejects non-id input %s",
    (raw) => {
