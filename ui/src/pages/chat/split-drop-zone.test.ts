...
describe("split drop zones", () => {
  ...
  it("copies non-enumerable DOMRect fields into full and half-pane indicators", () => {
    const domRect = Object.create(rect) as typeof rect;
    expect(splitDropIndicatorRect(domRect, { kind: "edge", edge: "left" })).toEqual({
      left: 100,
      top: 200,
      width: 200,
      height: 200,
    });
    expect(splitDropIndicatorRect(domRect, { kind: "edge", edge: "right" })).toEqual({
      left: 300,
      top: 200,
      width: 200,
      height: 200,
    });
    expect(splitDropIndicatorRect(domRect, { kind: "edge", edge: "up" })).toEqual({
      left: 100,
      top: 200,
      width: 400,
      height: 100,
